...
> **Делегирование.** Извлечение значений из таймлайна Figma Motion делегируй субагенту
> `proto-motion-extract` (recursive `get_motion_context`, разбор граблей выше): он вернёт
> трассируемую таблицу `[элемент → initial/animate/transition, times, ease, durationMs,
> источник]`. Впиши её в HANDOFF.md §Анимации и реализуй по ней; каждый помеченный
> `MISSING` — **остановись и спроси пользователя**. Правила источника выше — в силе.
...
Отметь «Анимация готова» в §«Статус фаз» и **прогони `review-animations`** на диффе
анимации — скилл вызывается моделью. Вердикт **Block** → исправить и перепрогнать; фаза
закрыта только при **Approve** (отметить в «Статус фаз»).
