...
Если наружу нужна **спека без кода** (натив-передача, прототип с непубличными ассетами) —
`DOC_ONLY_APPS`: уходит только `HANDOFF.md` в `handoff/<name>/`, исходники и `NOTES.md`
остаются приватными. Так отданы `mail-scroll` / `mail-scroll-v2`.
...
Перед `publish` проверить разложенное зеркало: нет исходников/ассетов непубличных прототипов,
нет внутренних идентификаторов, ссылки внутри HANDOFF не ведут на неопубликованные файлы.
Скрипт всегда льёт **полный снапшот** — если зеркало давно не синкали, наружу уедет и всё
накопленное, а не только правка текущей задачи.
