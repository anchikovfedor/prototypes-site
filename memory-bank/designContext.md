...
## Code Connect (опционально, для прототипов не нужен)
...
**Решение 2026-07-29: не настраиваем.** В репо его нет (ни `figma.config.json`, ни
`*.figma.tsx`), и для прототипов он мало даёт: Code Connect мапит компоненты **дизайн-системы**
на компоненты кода, а прототип пишет локальные компоненты под одну задачу — маппить нечего.
Маппинг фиксируем текстом в `apps/<name>/HANDOFF.md` §«Маппинг компонентов» (так сделано в
`documents-css`, `cards-drum`, `mail-scroll`). Если Figma MCP предложит подключить — вопрос
надо задать пользователю дословно, но дефолтный ответ «нет, как раньше».

## Токены макетов ≠ тема репозитория — сверять, а не доверять

Макеты разных задач собраны на разных наборах токенов, и значения расходятся со
сгенерированной темой Paradigm (`packages/kit/src/theme/paradigm.css`). Порядок: тянуть
`get_variable_defs` по узлу, сравнить с темой, расходящиеся переопределить **scoped** на
корневом элементе прототипа (не править `paradigm.css` — он общий).

Пример, <internal-ticket> (`apps/mail-scroll`): из 12 токенов макета разошлись два —
`--vkui--color_text_secondary` (#697481 в макете vs #87898F в теме) и
`--vkui--color_text_tertiary` (#939eac vs #AAADB3).

**Расхождения живут не вечно.** В обновлённом кадре того же файла (30.07) и письма, и виджеты
уже отдают `#87898f` — то есть значение темы, а scoped-переопределение в `.mail-screen` стало
лишним. При каждом обновлении макета перетягивать `get_variable_defs` заново и сверять, а не
доверять записанному списку.

Отдельная грабля: **VKUI `Tabbar`** рендерит подпись как `Footnote weight="2"`, а
`--vkui--font_weight_base2` в теме репозитория = 700; если в макете подпись Regular —
переопределить токен на контейнере.
